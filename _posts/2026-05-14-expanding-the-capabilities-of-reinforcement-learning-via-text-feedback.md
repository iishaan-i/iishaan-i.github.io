...
This is used as an auxiliary loss with <span class="math-render" data-display="inline" data-math="J_\text{MultiTurn}(\pi)"></span>, so intuitively, the whole training process likely encourages the model to follow the same process of critiquing its own work (by modeling the human-model interaction) and internalize the reasoning behind the feedback (which is likely necessary to model the feedback). This empirically works the best among all methods tested in this paper, which is a fascinating finding and the reason I rated this paper a 6. 
